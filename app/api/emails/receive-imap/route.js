  ...
  try {
    ...
    if (!user.mailplugSmtpUser || !user.mailplugSmtpPassword) {
      return NextResponse.json(
        {
          error:
            "Mailplug credentials not configured. Please set up your Mailplug settings first.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          details:
            "The operation took too long to complete. Try reducing the number of emails to fetch.",
          method: "IMAP",
          suggestion:
            "Reduce limit to 5 emails or check your network connection.",
        },
        { status: 408 }
      );
    }
