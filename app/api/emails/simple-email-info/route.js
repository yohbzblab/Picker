        ...
        tlsSocket.on('secureConnect', () => {
          ...
          tlsSocket.on('data', (data) => {
            ...
            if (currentStep === 'initial' && response.includes('+OK')) {
              ...
              console.log('📋 LIST 명령 결과:', response.trim().substring(0, 100))
              ...
              if (response.includes('+OK') && result.emailCount > 0) {
                ...
                const startIndex = Math.max(1, result.emailCount - maxEmails + 1)

                for (let i = result.emailCount; i >= startIndex && emailsToFetch.length < maxEmails; i--) {
                  emailsToFetch.push(i)
                }

                console.log(`📧 ${emailsToFetch.length}개 메일 헤더 가져오기 시작...`)

                result.steps.push({
                  step: 6,
                  action: 'LIST command successful, starting email header fetch',
                  status: 'success',
                  data: {
                ...
                if (emailsToFetch.length > 0) {
                  ...
                  console.log(`📧 메일 ${emailIndex} 헤더+미리보기 가져오기... (${currentFetchIndex + 1}/${emailsToFetch.length})`)
            ...
            } else if (currentStep === 'fetching_emails' && (response.includes('+OK') || response.includes('-ERR'))) {
              console.log(`📬 메일 ${emailsToFetch[currentFetchIndex]} 헤더+미리보기 응답 수신`)

              if (response.includes('+OK')) {
                // 메일 헤더와 미리보기 파싱
                const emailData = parseEmailHeader(response, emailsToFetch[currentFetchIndex])
                if (emailData) {
                  fetchedEmails.push(emailData)
                  console.log(`✅ 메일 ${emailsToFetch[currentFetchIndex]} 파싱 성공: ${emailData.subject}`)
                }
              } else {
                console.log(`❌ 메일 ${emailsToFetch[currentFetchIndex]} 가져오기 실패`)
              }
              ...
              if (currentFetchIndex < emailsToFetch.length) {
                ...
                result.steps.push({
                  ...
                  data: {
                    ...
                    emails: fetchedEmails.slice(0, 3).map(e => ({ subject: e.subject, from: e.from }))
                  },
                  timestamp: new Date().toISOString()
                })

                console.log(`✅ 전체 ${fetchedEmails.length}개 메일 전체 가져오기 완료`)
                console.log('👋 QUIT 명령 전송...')
                tlsSocket.write('QUIT\r\n')
                result.final_status = 'completed'
...
function parseFullEmail(response, emailIndex) {
  try {
    ...
    if (response.length > 50000) {
      console.log(`📏 메일 ${emailIndex}이 너무 큼 (${Math.round(response.length/1024)}KB), 처음 50KB만 처리`)
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inHeader) {
        ...
        if (line.toLowerCase().startsWith('content-type:')) {
          if (line.toLowerCase().includes('text/html') || line.toLowerCase().includes('multipart/alternative')) {
            isHtml = true
    ...
    return {
      ...
      isSpam: isSpam,
      hasAttachments: content.toLowerCase().includes('content-disposition: attachment'),
      preview: content ? extractTextPreview(content, isHtml).substring(0, 200) : subject,
...
function parseEmailHeader(response, emailIndex) {
  try {
    ...
    console.log(`🔍 메일 ${emailIndex} 파싱 시작, 전체 라인: ${lines.length}개`)
    ...
    return str.replace(/=\?([^?]+)\?([BQ])\?([^?]+)\?=/gi, (match, charset, encoding, text) => {
      try {
        if (encoding.toUpperCase() === 'B') {
          ...
          const decoded = text.replace(/_/g, ' ').replace(/=([0-9A-F]{2})/gi, (_, hex) => {
