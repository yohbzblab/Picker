...
const connectionOptions = {
  ...
  // Log only errors (query logging disabled for performance)
  log: ['error'],
};
...
export function getPrismaPickerClient() {
  if (!process.env.PICKER_DATABASE_URL) {
    // Avoid crashing at import/build time; fail only if actually used.
    throw new Error('Missing env var: PICKER_DATABASE_URL');
  }
